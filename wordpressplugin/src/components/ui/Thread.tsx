...
const Composer: React.FC<ComposerProps> = ({
  ...
  const {
    ...
    onTranscript: (text, isFinal) => {
      if (isFinal && text.trim()) {
        // Append the transcript to the composer
        const currentText = composerRuntime.getState().text || '';
        composerRuntime.setText(currentText + (currentText ? ' ' : '') + text.trim());
        clearTranscript();
      }
    },
  });
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      ...
      // Upload files if any
      if (attachments.length > 0 && apiUrl && instanceId) {
        try {
          const uploadedFiles = await uploadFiles(attachments, instanceId, apiUrl);
  ...
  }, [attachments, apiUrl, instanceId, isUploading, threadRuntime, composerRuntime]);
  ...
  const showVoiceButton = showVoiceInput && voiceSupported;
  ...
  return (
    <ComposerPrimitive.Root className="aui-composer-root">
      ...
      {isListening && transcript && (
        <div className="flowchat-voice-transcript">
          {transcript}
        </div>
      )}
      ...
      <div className="flowchat-composer-container">
        ...
        <div className="flowchat-composer-actions">
          {showVoiceButton && (
            <button
              type="button"
              className={`flowchat-voice-button ${isListening ? 'flowchat-voice-listening' : ''}`}
...
const LoadingIcon: React.FC<{ style?: React.CSSProperties }> = ({ style }) => (
  <svg style={style} viewBox="0 0 24 24" fill="currentColor" className="flowchat-loading-spinner">
